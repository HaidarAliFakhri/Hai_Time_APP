...
DateTime? _parseTanggal(String? tanggalStr, {String? waktu}) {
  ...
  final formats = [
    ...
    DateFormat("yyyy-MM-dd'T'HH:mm:ss"), // ISO-like with T
    DateFormat("dd/MM/yyyy"),
    DateFormat("dd-MM-yyyy"),
    DateFormat.yMd(), // fallback locale
  ];
  ...
  // kalau tanggalStr mungkin berisi timestamp (millis) atau iso langsung
  try {
    // coba parse langsung DateTime.parse (ISO)
    return _normalize(DateTime.parse(tanggalStr));
  } catch (_) {}

  // fallback: coba pakai createdAt (waktu) jika disediakan
...
String _eventsCacheKey = ""; // simple fingerprint
...
void _buildEventsCache(List<KegiatanFirebase> list) {
  ...
  for (final k in list) {
    if (_isKegiatanSelesaiModel(k)) continue; // skip item selesai

    final parsed = _parseTanggal(k.tanggal, waktu: k.createdAt);
    if (parsed == null) continue; // skip bila tanggal tak bisa parse
